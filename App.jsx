...
function App() {
  ...
  const greetUser = async () => {
    ...
    try {
      // Try to use the pre-generated greeting audio from backend
      await playBackendAudio(`${backendUrl}/shevanta_greeting.wav`)
    } catch (error) {
  ...
  const handleSpeechInput = async (text) => {
    ...
    try {
      ...
      if (data.audio_url) {
        await playBackendAudio(`${backendUrl}${data.audio_url}`)
      } else {
